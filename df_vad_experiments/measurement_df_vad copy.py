...
def visualize_and_save(original_path, enhanced_path, output_png, vad_model=None):
    ...
    # VAD 스코어 계산
    vad_times1, vad_scores1 = None, None
    vad_times2, vad_scores2 = None, None
    
    if vad_model is not None:
        # VAD는 48kHz 입력 필요
        y1_48k, _ = librosa.load(original_path, sr=48000)
        y2_48k, _ = librosa.load(enhanced_path, sr=48000)
        
        print("🎤 원본 오디오 VAD 스코어 계산 중...")
        vad_times1, vad_scores1 = compute_vad_scores(y1_48k, 48000, vad_model)
    ...
    # --- 3. VAD 스코어 시각화 ---
    if vad_model is not None and vad_times1 is not None:
        # 원본 VAD
        ax5 = plt.subplot(rows, 2, 5)
        ax5.fill_between(vad_times1, 0, vad_scores1, alpha=0.5, color='gray', label='Speech Probability')
        ax5.plot(vad_times1, vad_scores1, color='black', linewidth=0.8)
        ax5.axhline(y=0.5, color='red', linestyle='--', linewidth=1, label='Threshold (0.5)')
        ax5.set_xlim(0, len(y1) / sr1)
        ax5.set_ylim(0, 1)
        ax5.set_xlabel('Time (s)')
        ax5.set_ylabel('Speech Probability')
        ax5.set_title('Original VAD Score (Silero)')
        ...
        ax6.set_xlim(0, len(y2) / sr2)
        ax6.set_ylim(0, 1)
        ax6.set_xlabel('Time (s)')
        ax6.set_ylabel('Speech Probability')
        ax6.set_title('Enhanced VAD Score (Silero)')
