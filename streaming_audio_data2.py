...
def main():
    ...
    fig.suptitle('Real-time Audio Analysis (with Frequency Filtering)', fontsize=14, fontweight='bold')
    ...
    line_low_freq, = axes[4].plot(time_axis, np.zeros(BUFFER_SIZE),
                                   color='red', linewidth=0.5)
    axes[4].set_ylabel('Amplitude')
    axes[4].set_title(f'5. Low Frequency (< {LOW_FREQ_CUTOFF} Hz)')
    axes[4].set_xlim([0, BUFFER_SECONDS])
    axes[4].set_ylim([-1, 1])
    axes[4].grid(True, alpha=0.3)

    # 6. 고주파 (High Frequency) - 2000Hz 이상
    line_high_freq, = axes[5].plot(time_axis, np.zeros(BUFFER_SIZE),
                                    color='cyan', linewidth=0.5)
    axes[5].set_xlabel('Time (s)')
    axes[5].set_ylabel('Amplitude')
    axes[5].set_title(f'6. High Frequency (> {HIGH_FREQ_CUTOFF} Hz)')
    axes[5].set_xlim([0, BUFFER_SECONDS])
    axes[5].set_ylim([-1, 1])
    ...
    def update(frame):
        ...
        return line_waveform, line_rms, line_pressure, line_intensity, line_low_freq, line_high_freq
